...
char USAGE[] = "Usage: [options] " \
               ...
               "              or \n\n"\
               "[source_addr:]source_port UNIX:target_path\n\n" \
...
void proxy_handler(ws_ctx_t *ws_ctx) {
    ...
    /* Resolve target address */
    ...
    } // end of if target port
    else {   // UNIX domain port
      ...
      bzero((char *) &taddru, sizeof(taddru));
      taddru.sun_family = AF_UNIX;
      if(target_path[0]=='\0' ) {
        taddru.sun_path[0]='\0';
        strncpy(taddru.sun_path+1,target_path+1,sizeof(taddru.sun_path)-2);
      }
      else {
        strncpy(taddru.sun_path,target_path,sizeof(taddru.sun_path)-1);
      }
      if (connect(tsock, (struct sockaddr *) &taddru, sizeof(taddru)) < 0) {
        handler_emsg("Could not connect to target: %s\n",
                     strerror(errno));
        close(tsock);
        return;
      }
    }   // end of UNIX domain port
...
int dummymain(int argc, char *argv[])
{
    ...
    if (found) {
        ...
        if(strcmp(target_host,"UNIX")==0) {
         target_port=-1;
         strcpy(target_path,found+1);
         printf("using UNIX DOMAIN SOCKET: %s:%s\n",target_host,target_path);
        }
        else {
