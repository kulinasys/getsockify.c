...
int http(int childfd,char * header);
...
ws_ctx_t *do_handshake(int sock) {
    ...
    if (len == 0) {
        ...
        scheme = "wss";
        handler_msg("using SSL socket %s %s\n",settings.cert, settings.key);
...
//    handler_msg("handshake:\n %s\n", handshake);
    ret = parse_handshake(ws_ctx, handshake);
    if (ret == 0) {
        handler_msg("Not WS request:  http request\n");
// Need to Process http/https 
        process_req(ws_ctx,handshake);
        return NULL;
    }
    else if(ret<0) {
        handler_emsg("Invalid WS/HTTP request\n");
        free_ws_ctx(ws_ctx);
        return NULL;
    }
    headers = ws_ctx->headers;
    handler_msg("WS request: |%s|\n",headers->path)
